...
function ChangeOptionPosition( steps )
{
	...
	$('"'+"#selectList option[index='"+iActualIndex+"']"+'"').remove();
...
function RemoveSelectedOptions()
{
	...
	$("#txtText").attr("value","");
	$("#txtVal").attr("value","");
	$("#selectList option[index='"+iSelectedIndex+"']").remove();
	if($("#selectList option").length>0){
		if(iSelectedIndex >=$("#selectList option").length){
			iSelectedIndex=$("#selectList option").length-1;
		}
		$("#selectList option[index='"+iSelectedIndex+"']").attr("selected", true); 
		$("#txtText").attr("value",$("#selectList option[index='"+iSelectedIndex+"']").text());
		$("#txtVal").attr("value",$("#selectList option[index='"+iSelectedIndex+"']").attr("value"));
	}
}
