...
KE.util = {
	...
	addInstanceClickEvent : function(id) {
		var g = KE.g[id];
		if (g.instanceStack.length == 0) return;
		if(KE.browser.IE){
			KE.event.add(g.iframeDoc, 'mouseup', function(e){
				KE.util.selection(id);
				for (var i = 0, len = g.instanceStack.length; i < len; i++) {
					var item = g.instanceStack[i];
					if (item.click(id)) {
						break;
					}
				}
			...
			KE.event.add(g.iframeDoc, 'mousedown', function(e){
				KE.util.selection(id);
				for (var i = 0, len = g.instanceStack.length; i < len; i++) {
					var item = g.instanceStack[i];
					if (item.click(id)) {
...
KE.dialog = function(arg){
	...
	this.show = function() {
		...
		if (arg.html !== undefined) {
			...
			} else if(arg.file.substring(0,4)=="jsp_"){
				param = (/\?/.test(arg.file) ? '&' : '?') + param;
				iframe.src =   arg.file.substring(4) + param;
			}else {
				param = (/\?/.test(arg.file) ? '&' : '?') + param;
...
KE.superTag = function(id){
	var g = KE.g[id];
//	var startTag = "<"+g.newlineTag.toLowerCase() +"> ";
//	var endTag = " </"+g.newlineTag.toLowerCase()+">";
	var startTag = "<span>";
	var endTag = "</span>";
	var html = jQuery.trim(KE.html(id));
	if(html.substring(0, startTag.length)!=startTag||html.substring(html.length-endTag.length, html.length)!=endTag){
		KE.html(id ,startTag+html+endTag);
	}
};
...
KE.create = function(id, mode) {
	...
	KE.util.addInstanceClickEvent(id);
