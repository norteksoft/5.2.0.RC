...
$.extend($.jgrid,{
	...
	edit : {
		...
		bExit : "取消",
		msg: {
			required:"此字段必填",//ldx 之前为"此字段必需"，改为"此字段必填"
			number:"请输入有效数字",
			minValue:"输值必须大于等于 ",
			maxValue:"输值必须小于等于 ",
...
//grid.locale-cn.js中的内容end

//jquery.jqGrid.src.js中的内容start
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		$("th",thr).each(function ( j ) {
			...
			if( !(nm == 'cb' || nm=='subgrid' || nm=='rn') ) {
				if(ts.p.viewsortcols[2]){
					//$(">div",this).addClass('ui-jqgrid-sortable');//dzy,把该行修改为下面一行，是解决表头无法排序的问题。因为在属性名含点（例如：o.name）时,$(">div",this)为空。
					$("div[id='jqgh_"+ts.p.id+"_"+ts.p.colModel[j].name+"']").addClass('ui-jqgrid-sortable');
				}
			}
			if(sort) {
			...
			//sortData( $('div',this)[0].id, ci, r, d);//dzy,把该行修改为下面一行，是解决表头无法排序的问题。因为在属性名含点（例如：o.name）时,$('div',this)为空。
			sortData( 'jqgh_'+ts.p.id+"_"+ts.p.colModel[ci].name, ci, r, d);
			return false;
		});
		...
		.dblclick(function(e) {
			...
			if(this.p!=undefined)//dzy 增加if(this.p!=undefined)是解决：在冻结列上双击鼠标报找不到this.p.onRightClickRow脚本错误，是因为this.p=undefined
		...
		.bind('contextmenu', function(e) {
			...
			if(this.p!=undefined)//dzy 增加if(this.p!=undefined)是解决：在冻结列上点击鼠标右键报找不到this.p.onRightClickRow脚本错误，是因为this.p=undefined
...
//Helper functions
	findPos : function(obj) {
		...
		//dzy 减去垂直滚动条滚动的长度 start
		var tds = $(orignalObj).find("td");
		var gridId=($(tds[0]).attr("aria-describedby")).split("_")[0];
		var scrollTop=$("#"+gridId).closest(".ui-jqgrid-bdiv").scrollTop();
		curtop=curtop-scrollTop;
		//dzy 减去垂直滚动条滚动的长度 end
